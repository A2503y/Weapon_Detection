...
OUTPUT_DIR = "C:\\Users\\Mehak\\OneDrive\\Documents\\weapon_detection_project\\output"
...
def get_weapon_classes(config):
    ...
    for category in config["enabled_categories"]:
        if category in WEAPON_CATEGORIES:
            weapon_classes.extend(WEAPON_CATEGORIES[category])
    
    # Add any custom classes
    for category, classes in config.get("custom_classes", {}).items():
        if category in config["enabled_categories"]:
            weapon_classes.extend(classes)
